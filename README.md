...
내 게임의 현재 동접자 수를 주기적으로 관찰해서, **직전 알림값에서 일정 폭 이상 변할 때** 지정 채널로 알려준다. 데이터는 Steam 공식 API에서 가져오므로 API 키가 필요 없다.
...
| `STEAM_ALERT_THRESHOLD` | `5` | 직전 알림값 대비 ±이만큼 변하면 알림 |
| `STEAM_ALERT_MIN_COUNT` | `10` | 현재 동접이 이 값 이상일 때만 알림 |

> 봇 재시작 직후 첫 조회는 기준선만 잡고 알림을 보내지 않는다(스팸 방지). 이후 `|현재값 − 직전알림값| ≥ THRESHOLD` 이고 `현재값 ≥ MIN_COUNT` 이면 알림 후 기준선을 갱신한다.
